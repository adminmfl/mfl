...
function getDefaultValues(measurementType: string): {
  min: number;
  max: number;
} {
...
export function ActivityMinimumDropdown({
  ...
  const [baseMin, setBaseMin] = useState<number | null>(
    initialConfig?.min_value ?? null,
  );
  ...
  useEffect(() => {
    setProofDraft(proofRequirement);
  }, [proofRequirement]);
  useEffect(() => {
    setNotesDraft(notesRequirement);
  }, [notesRequirement]);
  useEffect(() => {
    setPointsDraft(pointsPerSession);
  }, [pointsPerSession]);
  useEffect(() => {
    setOutcomeDraft(
      outcomeConfig && outcomeConfig.length > 0 ? outcomeConfig : [],
  ...
  async function handleSave() {
    if (
      ageOverrides.some(
        (override) =>
          override.ageMin === null ||
          override.ageMax === null ||
      ...
      onActivityConfigChange({
        ...
        outcome_config:
          outcomeDraft.length > 0
            ? outcomeDraft.filter((o) => o.label.trim())
            : null,
      ...
      {isExpanded && (
        <div className="mt-3 space-y-3 p-3 bg-muted/30 rounded-lg border border-border/60">
          ...
          <div className="space-y-2">
            ...
            <h5 className="text-xs font-medium text-foreground/80">
              Base Tier (All Ages)
            </h5>
            <p className="text-xs text-muted-foreground">
              Set minimum only. Maximum auto-calculated as minimum × 2 (RR:
              1.0-2.0)
            </p>
          ...
          <div className="space-y-2 border-t pt-3">
            ...
            <p className="text-xs text-muted-foreground bg-muted/30 p-2 rounded border border-border/60">
              Set different minimum requirements for specific age groups. For
              example: "Ages 0-20 need 30-60 minutes" and "Ages 60+ need 20-40
              minutes". Everything in between uses the base tier.
            </p>
          ...
          <div className="space-y-2 border-t pt-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Select
                  value={proofDraft}
                  onValueChange={(v) => setProofDraft(v as any)}
                >
              ...
              <div>
                ...
                <Select
                  value={notesDraft}
                  onValueChange={(v) => setNotesDraft(v as any)}
                >
                  <SelectTrigger className="h-7 text-xs">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Select
                  value={String(maxImagesDraft)}
                  onValueChange={(v) => setMaxImagesDraft(Number(v))}
                >
                  <SelectTrigger className="h-7 text-xs">
                    <SelectValue />
                  </SelectTrigger>
          ...
          <div className="space-y-2 border-t pt-3">
            ...
            <p className="text-xs text-muted-foreground">
              Add outcomes like Win/Loss/Draw with different point values. Leave
              empty for single-outcome activities.
            </p>
            {outcomeDraft.length === 0 ? (
              <p className="text-xs text-muted-foreground">

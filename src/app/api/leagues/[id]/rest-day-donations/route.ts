...
export async function GET(
  ...
  try {
    ...
    if (roleNames.includes('host')) userRole = 'host';
      ...
      roleNames.includes('captain') ||
      roleNames.includes('vice_captain')
    )
      userRole = 'captain';
...
export async function POST(
  ...
  try {
    ...
    const { receiver_member_id, days_transferred, notes, proof_url } =
      parsed.data;
    ...
    if (receiverError || !receiverMembership) {
      return NextResponse.json(
        { error: 'Receiver not found in this league' },
        { status: 400 },
      );
    }
    ...
    const restDayQueryWithDate = league.start_date
      ? restDayQuery.gte('date', league.start_date)
      : restDayQuery;
    ...
    const finalRemaining = Math.max(
      0,
      totalAllowed + received - donated - (approvedRestDays || 0),
    );

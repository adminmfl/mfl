...
import { getLeagueById, deriveLeagueStatus } from '@/lib/services/leagues';
...
export async function GET(
  ...
  try {
    ...
    // 1. Fetch search params for week range
    ...
    // 2. Fetch League and Member details in parallel
    ...
    const filterStartDate = startDateParam || league.start_date;
    const filterEndDate = endDateParam || league.end_date;
    ...
    if (league.status === 'completed') {
      ...
      const cutoff = addDaysYYYYMMDD(todayYmd, -2);
      summaryEndDate = todayYmd;
      
      // Compute pending window range
      const pendingWindowEnd = minDateString(league.end_date, todayYmd);
      const pendingWindowStart = addDaysYYYYMMDD(pendingWindowEnd, -1);
      
      pendingWindowDates = [pendingWindowEnd, pendingWindowStart]
        .filter(d => d >= league.start_date && d <= league.end_date)
        .filter(d => d > cutoff)
        .sort();
    }

    // 3. Parallel Fetching for stats, submissions, rest days, team info, activities
    const [
      statsRes,
      submissionsRes,
      restRes,
      ...
      supabase.rpc('get_league_participation_stats', { p_league_id: leagueId }),
      supabase
        .from('effortentry')
        .select(`
      ...
      teamId ? supabase
        ...
        .eq('team_id', teamId)
        .eq('league_id', leagueId) : Promise.resolve({ data: null }),
      supabase
    ...
    // Deduplicate challenge subs: one per challenge per member
    ...
    // Personal challenge points (not added to individual total in some formulas, but let's calculate)
    ...
    // 8. Grid Row Generation (Recent Days)
    ...
    // Use similar logic to the frontend for week/month boundaries
    ...
    for (let offset = 0; offset < numDays; offset++) {
      ...
      if (isMonthly) {
        ...
        if (isAfterEnd) {
          ...
          // pick latest approved or latest overall
    ...
    // 9. Team Ranking & Points (Simplified version of leaderboard route logic)
    const cutoffDate = league.status === 'completed' ? league.end_date : addDaysYYYYMMDD(todayYmd, -2);
    
    // We need points for ALL teams in the league to determine RANK
    const { data: allLeagueMembers } = await supabase
      .from('leaguemembers')
      .select('league_member_id, team_id')
      .eq('league_id', leagueId);
    ...
    const { data: allApprovedEntries } = await supabase
      .from('effortentry')
      .select('league_member_id, date, rr_value, workout_type, outcome, status, type')
      .in('league_member_id', (allLeagueMembers || []).map(m => m.league_member_id))
      .eq('status', 'approved')
      .lte('date', cutoffDate);
    ...
    (allLeagueMembers || []).forEach(m => {
      if (m.team_id) {
        if (!teamMemberMap.has(m.team_id)) teamMemberMap.set(m.team_id, new Set());
        teamMemberMap.get(m.team_id)!.add(m.league_member_id);
      }
    });

    // Deduplicate all entries for team point calculation
    ...
    (allLeagueMembers || []).forEach(m => { if (m.team_id) memberToTeam.set(m.league_member_id, m.team_id); });
    ...
    // Add challenge points to teams
    uniqueChallengeSubs.forEach(s => {
      if (s.team_id) {
        const current = teamPointsMap.get(s.team_id) || 0;
        teamPointsMap.set(s.team_id, current + (s.awarded_points ?? s.leagueschallenges.total_points ?? 0));
      }
    });

    // Rank teams
    const teamRankings = Array.from(teamMemberMap.keys()).map(tId => ({
      team_id: tId,

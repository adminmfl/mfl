...
export async function PATCH(
  ...
  try {
    ...
    const hasFrequencyType = Object.prototype.hasOwnProperty.call(
      body,
      'frequency_type',
    );
    const hasProofRequirement = Object.prototype.hasOwnProperty.call(
      body,
      'proof_requirement',
    );
    const hasNotesRequirement = Object.prototype.hasOwnProperty.call(
      body,
      'notes_requirement',
    );
    const hasPointsPerSession = Object.prototype.hasOwnProperty.call(
      body,
      'points_per_session',
    );
    const hasOutcomeConfig = Object.prototype.hasOwnProperty.call(
      body,
      'outcome_config',
    );
    const hasMaxImages = Object.prototype.hasOwnProperty.call(
      body,
      'max_images',
    );
    const hasCustomFieldLabel = Object.prototype.hasOwnProperty.call(
      body,
      'custom_field_label',
    );
    const hasCustomFieldPlaceholder = Object.prototype.hasOwnProperty.call(
      body,
      'custom_field_placeholder',
    );
    const hasCustomFieldLabel2 = Object.prototype.hasOwnProperty.call(
      body,
      'custom_field_label_2',
    );
    const hasCustomFieldPlaceholder2 = Object.prototype.hasOwnProperty.call(
      body,
      'custom_field_placeholder_2',
    );
    ...
    if (
      ...
      !hasCustomFieldLabel2 &&
      !hasCustomFieldPlaceholder2
    ) {
      return NextResponse.json(
    ...
    if (hasFrequencyType) {
      if (
        ...
        return NextResponse.json(
          {
            error:
              'frequency_type must be daily, weekly, or monthly (or null to reset)',
          },
          { status: 400 },
        );
      } else {
        normalizedFrequencyType = frequency_type as
          | 'daily'
          | 'weekly'
          | 'monthly';
      }
      ...
      if (frequency === null || frequency === undefined || frequency === '') {
        ...
        if (!hasFrequencyType) {
          const { data: currentRow } = await supabase
            ...
            .eq('league_id', leagueId)
            .or(
              `activity_id.eq.${activity_id},custom_activity_id.eq.${activity_id}`,
            )
            .maybeSingle();
        ...
        const maxAllowed =
          effectiveType === 'monthly' ? 28 : effectiveType === 'daily' ? 10 : 7;
        if (rounded === 0) {
          normalizedFrequency = null;
    ...
    if (hasProofRequirement) {
      ...
      if (!validValues.includes(proof_requirement as string)) {
        return NextResponse.json(
          {
            error:
              'proof_requirement must be not_required, optional, or mandatory',
          },
          { status: 400 },
        );
      }
    ...
    if (hasNotesRequirement) {
      ...
      if (!validValues.includes(notes_requirement as string)) {
        return NextResponse.json(
          {
            error:
              'notes_requirement must be not_required, optional, or mandatory',
          },
          { status: 400 },
        );
      }
    ...
    if (hasCustomFieldLabel) {
      updatePayload.custom_field_label = custom_field_label
        ? String(custom_field_label).trim()
        : null;
    }

    // Handle custom_field_placeholder
    if (hasCustomFieldPlaceholder) {
      updatePayload.custom_field_placeholder = custom_field_placeholder
        ? String(custom_field_placeholder).trim()
        : null;
    }

    // Handle custom_field_label_2
    if (hasCustomFieldLabel2) {
      updatePayload.custom_field_label_2 = custom_field_label_2
        ? String(custom_field_label_2).trim()
        : null;
    }

    // Handle custom_field_placeholder_2
    if (hasCustomFieldPlaceholder2) {
      updatePayload.custom_field_placeholder_2 = custom_field_placeholder_2
        ? String(custom_field_placeholder_2).trim()
        : null;
    }
    ...
    if (updateError) {
      ...
      if (message.includes('frequency') && message.includes('column')) {
        return NextResponse.json(
          {
            error: 'Frequency is not supported yet for this league',
            code: 'FREQUENCY_NOT_SUPPORTED',
          },
          { status: 409 },
        );
      }
...
export async function GET(
  ...
  try {
    ...
    if (activitiesError && typeof activitiesError?.message === 'string') {
      ...
      // Fallback: if proof_requirement/notes_requirement/points_per_session/outcome_config/max_images/custom_field_label/custom_field_placeholder/custom_field_label_2/custom_field_placeholder_2 columns don't exist
      ...
      const isNewColumnError =
        msg.includes('proof_requirement') ||
        msg.includes('notes_requirement') ||
        msg.includes('points_per_session') ||
        msg.includes('outcome_config') ||
        msg.includes('max_images') ||
        msg.includes('custom_field_label') ||
        msg.includes('custom_field_placeholder') ||
        msg.includes('custom_field_label_2') ||
        msg.includes('custom_field_placeholder_2');
      const isGenericColumnError =
        msg.includes('column') &&
      ...
      .map((la) => {
        ...
        if (la.custom_activities) {
          ...
          return {
            ...
            custom_field_placeholder:
              (la as any).custom_field_placeholder ?? null,
            custom_field_label_2: (la as any).custom_field_label_2 ?? null,
            custom_field_placeholder_2:
              (la as any).custom_field_placeholder_2 ?? null,
          };
        }
    ...
    if (includeAll && isHost) {
      ...
      if (customActs) {
        const customActivitiesForConfig: LeagueActivity[] = customActs.map(
          (ca: any) => ({
    ...
    if (enabledActivities.length === 0 && !includeAll) {
      return NextResponse.json({
        ...
        message:
          'This league has no activities configured. Please contact the league host.',
...
export async function POST(
  ...
  try {
    ...
    if (activity_ids.length > 0) {
      ...
      const validActivityIds = (existingActivities || []).map(
        (a) => a.activity_id,
      );
      const potentialCustomIds = activity_ids.filter(
        (id: string) => !validActivityIds.includes(id),
      );
      ...
      if (potentialCustomIds.length > 0) {
        ...
        const validCustomIdsFromMixed = (foundCustom || []).map(
          (c) => c.custom_activity_id,
        );
    ...
    if (
      custom_activity_ids &&
      Array.isArray(custom_activity_ids) &&
      custom_activity_ids.length > 0
    ) {
      // Verify ownership of custom activities
      const { data: customActivities, error: customVerifyError } =
        await supabase
          .from('custom_activities')
          .select('custom_activity_id')
      ...
      const validCustomIds = (customActivities || []).map(
        (ca) => ca.custom_activity_id,
      );
      const invalidCustomIds = custom_activity_ids.filter(
        (id: string) => !validCustomIds.includes(id),
      );
      ...
      const existingCustomIds = (existingCustom || []).map(
        (e) => e.custom_activity_id,
      );
      ...
      if (newCustomIds.length > 0) {
        const customInsertData = newCustomIds.map(
          (custom_activity_id: string) => ({
            league_id: leagueId,
            custom_activity_id,
            created_by: userId,
        ...
        if (customInsertError) {
          console.error(
            'Error adding custom activities to league:',
            customInsertError,
          );
          return NextResponse.json(
    ...
    if (
      (!activity_ids || activity_ids.length === 0) &&
      (!custom_activity_ids || custom_activity_ids.length === 0)
    ) {
      return NextResponse.json(

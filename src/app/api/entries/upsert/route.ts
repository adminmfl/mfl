...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      custom_field_value_2, // Second custom field value from host-configured custom_field_label_2
    ...
    const todayYmd = getUserLocalDateYMD({
      ...
      legacyTimezoneOffset:
        typeof timezone_offset === 'number' && Number.isFinite(timezone_offset)
          ? timezone_offset
          : null,
    });
    ...
    const { data: leagueRow, error: leagueRowError } = await supabase
      .from('leagues')
      .select('league_id, end_date, status, rr_config, league_mode')
    ...
    const leagueMode = (leagueRow as any).league_mode || 'standard';

    // Block activity submissions for challenges-only leagues
    if (leagueMode === 'challenges_only') {
      return NextResponse.json(
        {
          error:
            'This league uses challenges only. Activity submissions are not accepted.',
        },
        { status: 400 },
      );
    }

    // Fetch submission_window_days + frequency_type for this activity
    let submissionWindowDays: number | null = null;
    if (type === 'workout' && workout_type) {
      const isUuidWorkoutType =
      ...
      if (isUuidWorkoutType) {
        const { data } = await supabase
          ...
          .select('submission_window_days, frequency_type')
          .eq('league_id', league_id)
          .eq('custom_activity_id', workout_type)
          .maybeSingle();
        if (data) windowData = data;
      }
      ...
      if (!windowData) {
        const { data } = await supabase
          .from('leagueactivities')
          .select(
            'submission_window_days, frequency_type, activities!inner(activity_name)',
          )
          .eq('league_id', league_id)
      ...
      if (
        windowData &&
        typeof windowData.submission_window_days === 'number' &&
        windowData.submission_window_days > 0
      ) {
        // Host explicitly configured a window
        submissionWindowDays = windowData.submission_window_days;
      } else if (windowData) {
        // Smart defaults: calendar-based "end of period + 1 day" (per client requirement)
        // Weekly: entry allowed until end-of-week (Sunday) + 1 day (Monday)
        // Monthly: entry allowed until end-of-month + 1 day (1st of next month)
        // Daily: stays null → today/yesterday only
        const freqType = windowData.frequency_type;
        if (freqType === 'weekly' || freqType === 'monthly') {
          const entryParts = normalizedDate.split('-').map(Number);
          const entryD = new Date(
            Date.UTC(entryParts[0], entryParts[1] - 1, entryParts[2]),
          );
          ...
          let deadline: Date;
          if (freqType === 'weekly') {
            // End of ISO week (Sunday) containing entry_date, + 1 day
            const dayOfWeek = entryD.getUTCDay(); // 0=Sun, 1=Mon, ...
            const daysUntilSunday = dayOfWeek === 0 ? 0 : 7 - dayOfWeek;
            deadline = new Date(entryD);
            deadline.setUTCDate(deadline.getUTCDate() + daysUntilSunday + 1); // Monday after
          } else {
            // End of month containing entry_date, + 1 day (= 1st of next month)
            deadline = new Date(
              Date.UTC(entryD.getUTCFullYear(), entryD.getUTCMonth() + 1, 0),
            ); // last day of month
            deadline.setUTCDate(deadline.getUTCDate() + 1); // 1st of next month
          }

          if (todayD > deadline) {
            const label = freqType === 'weekly' ? 'the week' : 'the month';
            return NextResponse.json(
              {
                error: `Submission window has closed. You can submit until end of ${label} + 1 day.`,
              },
              { status: 400 },
            );
          }
          // Mark as handled so the rolling-window check below is skipped
          submissionWindowDays = -1;
        }
      }
    }
    ...
    if (leagueStatus === 'completed') {
      return NextResponse.json(
        { error: 'League has completed. Submissions are closed.' },
        { status: 400 },
      );
    }
    ...
    if (leagueEnd) {
      ...
      if (
        !reupload_of &&
        normalizedDate !== todayYmd &&
        normalizedDate !== yesterdayYmd
      ) {
        const isValidLateSubmission = normalizedDate === dateStr; // dateStr is leagueEnd (YYYY-MM-DD)

        if (!isValidLateSubmission) {
          // Check if a wider submission window is configured for this activity
          // submissionWindowDays === -1 means calendar-based check already passed above
          if (submissionWindowDays && submissionWindowDays > 0) {
            const windowStart = new Date(todayYmd + 'T00:00:00');
            windowStart.setDate(windowStart.getDate() - submissionWindowDays);
            const windowStartYmd = windowStart.toISOString().split('T')[0];
            if (normalizedDate < windowStartYmd) {
              return NextResponse.json(
                {
                  error: `You can only submit for the last ${submissionWindowDays} day${submissionWindowDays === 1 ? '' : 's'}.`,
                },
                { status: 400 },
              );
            }
          } else if (!submissionWindowDays) {
            return NextResponse.json(
    ...
    if (!type || !['workout', 'rest'].includes(type)) {
      return NextResponse.json(
        { error: "type must be 'workout' or 'rest'" },
        { status: 400 },
      );
    }
    ...
    if (
      typeof steps === 'number' &&
      (!Number.isFinite(steps) || steps < 0 || steps > 500000)
    ) {
      return NextResponse.json(
        { error: 'Steps must be between 0 and 500,000' },
        { status: 400 },
      );
    }
    if (
      typeof holes === 'number' &&
      (!Number.isFinite(holes) || holes < 0 || holes > 36)
    ) {
      return NextResponse.json(
        { error: 'Holes must be between 0 and 36' },
        { status: 400 },
      );
    }
    ...
    if (!membership.team_id) {
      return NextResponse.json(
        {
          error:
            'You must be assigned to a team to submit activities. Contact the league host.',
        },
        { status: 403 },
      );
    }
    ...
    if (type === 'workout' && workout_type && !reupload_of) {
      ...
      const rawFrequencyType =
        (leagueActivity as any)?.frequency_type ?? 'weekly';
      const frequencyType: 'daily' | 'weekly' | 'monthly' =
        rawFrequencyType === 'monthly'
          ? 'monthly'
          : rawFrequencyType === 'daily'
            ? 'daily'
            : 'weekly';
      resolvedFrequencyType = frequencyType;
      const normalizedFrequency =
        typeof rawFrequency === 'number' && Number.isFinite(rawFrequency)
          ? Math.floor(rawFrequency)
          : null;
      const frequency =
        typeof normalizedFrequency === 'number' && normalizedFrequency > 0
          ? normalizedFrequency
          : null;
      ...
      if (typeof frequency === 'number' && frequency >= 1) {
        if (frequencyType === 'daily') {
          ...
          if (dailyError) {
            ...
            return NextResponse.json(
              { error: 'Failed to validate activity limit' },
              { status: 500 },
            );
          }

          const activeCount = (dailyEntries || []).filter(
          ...
          if (!dateRange) {
            return NextResponse.json(
              { error: 'Invalid date format for submission' },
              { status: 400 },
            );
          }
    ...
    const rrConfig: RRConfig = leagueRRConfig || {
      formula: 'standard',
      age_adjustments: true,
    };
    const { baseDuration, minSteps, maxSteps } = getAgeThresholds(
      userAge,
      rrConfig,
    );
    ...
    if (reupload_of) {
      ...
      if (originalError) {
        ...
        return NextResponse.json(
          { error: 'Failed to validate resubmission' },
          { status: 500 },
        );
      }
      if (!original) {
        return NextResponse.json(
          { error: 'Original submission not found for resubmit' },
          { status: 404 },
        );
      }
      if (original.league_member_id !== membership.league_member_id) {
        return NextResponse.json(
          { error: 'You can only resubmit your own rejected submissions' },
          { status: 403 },
        );
      }
      ...
      if (originalDate && normalizedDate !== originalDate) {
        return NextResponse.json(
          { error: 'Resubmissions must use the original submission date' },
          { status: 400 },
        );
      }
      ...
      if (childError) {
        ...
        return NextResponse.json(
          { error: 'Failed to validate resubmission history' },
          { status: 500 },
        );
      }

      const hasApprovedResub = (childResubs || []).some(
      ...
      if (hasApprovedResub) {
        return NextResponse.json(
          {
            error:
              'This submission already has an approved resubmission. Further uploads are not allowed.',
          },
          { status: 409 },
        );
      }
    ...
    const { data: existingRows, error: existingError } =
      await existingQuery.order('created_date', { ascending: false });

    if (existingError) {
      console.error('Existing entry lookup error:', existingError);
      return NextResponse.json(
        { error: 'Failed to validate existing submissions' },
        { status: 500 },
      );
    }
    ...
    const hasNonRejected = (existingRows ?? []).some(
      (r: any) => r?.status && r.status !== 'rejected',
    );
    ...
    let activityProofRequirement: 'not_required' | 'optional' | 'mandatory' =
      'mandatory';
    let activityNotesRequirement: 'not_required' | 'optional' | 'mandatory' =
      'optional';
    let activityPointsPerSession: number = 1;
    let activityOutcomeConfig: { label: string; points: number }[] | null =
      null;
    ...
    if (type === 'workout' && workout_type) {
      ...
      if (isUuidWorkoutType) {
        ...
        const { data, error } = await supabase
          .from('leagueactivities')
          .select(
            'proof_requirement, notes_requirement, points_per_session, outcome_config, min_value',
          )
          .eq('league_id', league_id)
          .eq('custom_activity_id', workout_type)
          .maybeSingle();
      ...
      if (!activityConfigRow) {
        ...
        const { data, error } = await supabase
          .from('leagueactivities')
          .select(
            'proof_requirement, notes_requirement, points_per_session, outcome_config, min_value, activities!inner(activity_name)',
          )
          .eq('league_id', league_id)
          .eq('activities.activity_name', workout_type)
          .maybeSingle();
      ...
      if (activityConfigRow) {
        activityProofRequirement =
          activityConfigRow.proof_requirement ?? 'mandatory';
        activityNotesRequirement =
          activityConfigRow.notes_requirement ?? 'optional';
    ...
    if (
      type === 'workout' &&
      activityProofRequirement === 'mandatory' &&
      !proof_url &&
      !(canReplaceRejected && existing?.proof_url)
    ) {
      return NextResponse.json(
    ...
    if (
      type === 'workout' &&
      activityNotesRequirement === 'mandatory' &&
      !notes
    ) {
      return NextResponse.json(
    ...
    if (
      ...
      activityOutcomeConfig.length > 0
    ) {
      if (!outcome) {
        return NextResponse.json(
    ...
    // Calculate RR value based on activity type using shared calculator
    {
      ...
      if (
        activityConfigRow &&
        typeof activityConfigRow.min_value === 'number' &&
        activityConfigRow.min_value > 0
      ) {
    ...
    if (
      ...
      if (deleteAutoErr) {
        console.error(
          'Failed to remove auto-assigned rest day for late submission:',
          deleteAutoErr,
        );
      } else {
    ...
    const effectiveHasNonRejected = (existingRows ?? []).some(
      (r: any) => r?.status && r.status !== 'rejected',
    );
    const effectiveCanReplaceRejected =
      !!effectiveExisting && !effectiveHasNonRejected;
      ...
      if (overwrite || effectiveCanReplaceRejected) {
        ...
        return NextResponse.json({
          ...
          data: {
            ...updated,
            points_per_session:
              activityOutcomeConfig && outcome
                ? (activityOutcomeConfig.find((o: any) => o.label === outcome)
                    ?.points ?? activityPointsPerSession)
                : activityPointsPerSession,
          },
          updated: true,

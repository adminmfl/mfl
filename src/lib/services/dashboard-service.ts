...
export async function getDashboardSummary(
  ...
  const [
    ...
    supabase
      ...
      .select('activity_id, custom_activity_id, points_per_session, outcome_config, frequency_type, activities(activity_name)')
      .eq('league_id', leagueId),
    supabase
  ...
  const personalChallengePoints = uniqueChallengeSubs
    ...
    .reduce((sum, s) => sum + (s.awarded_points ?? (s.leagueschallenges as any)?.total_points ?? 0), 0);

  const teamChanPts = teamId ? uniqueChallengeSubs
    .filter(s => s.team_id === teamId)
    .reduce((sum, s) => sum + (s.awarded_points ?? (s.leagueschallenges as any)?.total_points ?? 0), 0) : 0;
  ...
  const { data: allApprovedEntries } = await supabase
    ...
    .in('league_member_id', (allLeagueMembers.data || []).map((m: any) => m.league_member_id))
    .eq('status', 'approved')
    .lte('date', cutoffDate);
  ...
  (allLeagueMembers.data || []).forEach((m: any) => { if (m.team_id) memberToTeam.set(m.league_member_id, m.team_id); });
  ...
  uniqueChallengeSubs.forEach(s => {
    if (s.team_id) {
      ...
      teamPointsMap.set(s.team_id, current + (s.awarded_points ?? (s.leagueschallenges as any)?.total_points ?? 0));
    }
  });
